...
router.post('/:userId/cart', ensureAuthenticated, function (req, res, next) {
  let userId = req.params.userId
  let { productId, increase, decrease, remove } = req.body
  ...
  Cart.getCartByUserId(userId, function (err, c) {
    ...
    // FAST PATH FOR REMOVE: don't validate product existence in Mongoose since it might be an orphaned/deleted product
    if (remove) {
      oldCart.removeItem(productId);
      return Cart.updateCartByUserId(
        userId,
